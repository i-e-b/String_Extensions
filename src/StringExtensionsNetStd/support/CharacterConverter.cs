...
internal static class CharacterConverter
{
    ...
    /// <summary>
    /// Convert characters to ASCII where possible.
    /// This will replace accented forms with ones that *look* similar, but it will often destroy or
    /// change meaning.
    /// <p/>
    /// Note: this can only support characters up to \uFFFF.
    /// </summary>
    public static string ConvertToAscii(char c)
    {
        return ConvertToAscii((int)c);
    }

    /// <summary>
    /// Convert Unicode points to ASCII where possible.
    /// This will replace accented forms with ones that *look* similar, but it will often destroy or
    /// change meaning.
    /// </summary>
    public static string ConvertToAscii(int i)
    {
        // Quick test: if its not in range then just keep current character
        if (i < 0x0080) return char.ConvertFromUtf32(i);

        if (i is > 0xE0000 and < 0xE0080) // Unicode tag block ( https://en.wikipedia.org/wiki/Tags_(Unicode_block) )
        {
            return char.ConvertFromUtf32(i - 0xE0000);
        }
